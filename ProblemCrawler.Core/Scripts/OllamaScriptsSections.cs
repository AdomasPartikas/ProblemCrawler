...
public static class OllamaScriptSections
{
    ...
    [
        ...
        $"  {Sudo(password, "mkdir -p /usr/lib/ollama/rocm/rocblas/library/")}",
        $"  {Sudo(password, $"bash -c 'find {rocm.RocmLibPath}/rocblas/library/ -type f -name \"*${{GFX_TARGET}}*\" -exec cp {{}} /usr/lib/ollama/rocm/rocblas/library/ \\;'")}",
        ...
        "HSA_DST=$(find /usr/lib/ollama/ -path '*/rocm_v*/*' -name 'libhsa-runtime64.so.1.*' ! -type l | head -1)",
        $"echo \"[ollama] Copying $HSA_SRC -> $HSA_DST\"",
        $"[ -n \"$HSA_SRC\" ] && [ -n \"$HSA_DST\" ] && {Sudo(password, "cp \"$HSA_SRC\" \"$HSA_DST\"")} || echo '[ollama] WARNING: HSA src/dst not found'",
    ...
    [
        ...
        "unset OLLAMA_VULKAN",
        "ROCM_LIB=$(basename $(ls -d /usr/lib/ollama/rocm_v* 2>/dev/null | sort -V | tail -1) 2>/dev/null || echo rocm)",
        "echo \"[ollama] Using LLM library: $ROCM_LIB\"",
        "export OLLAMA_LLM_LIBRARY=$ROCM_LIB",
        ...
        $"export LD_LIBRARY_PATH=/usr/lib/ollama/$ROCM_LIB:{svc.OllamaLibraryPath}:{rocm.RocmLibPath}:${{LD_LIBRARY_PATH}}",
    ...
    [
        ...
        Sudo(password, $"systemctl disable ollama 2>/dev/null || true"),
        "sleep 2",   
        Sudo(password, $"killall ollama 2>/dev/null || true"),   
        "sleep 1",
        Sudo(password, $"killall -9 ollama 2>/dev/null || true"), 
        Sudo(password, $"fuser -k {svc.Port}/tcp 2>/dev/null || true"),
        "sleep 2"
